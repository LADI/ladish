...
bool event::append_sysex(unsigned char *a_data, long a_size)
{
	bool
	ret = true;

	unsigned char *
	buffer = new unsigned char[m_size + a_size];
...
bool event::is_note_on()
{
	return (m_status == EVENT_NOTE_ON);
}

bool event::is_note_off()
{
...
bool event::operator>(const event & a_rhsevent)
{
...
bool event::operator<=(const unsigned long &a_rhslong)
{
