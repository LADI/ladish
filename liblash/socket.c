...
lash_open_socket (int * sockptr, const char * host, const char * service)
{
  ...
  LASH_DEBUGARGS ("attempting to connect to host '%s', service '%s'", host, service);
  ...
  for (addr = addrs; addr; addr = addr->ai_next)
	...
	if (err)
	  {
	    fprintf (stderr, "%s: error looking up numeric host/port names: %s\n",
		     __FUNCTION__, strerror (errno));
	  }
	else
	  {
	    LASH_DEBUGARGS ("attempting to connect to %s:%s", num_addr, port_addr);
	  }
      }
#endif
