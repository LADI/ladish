  ...
  for (i = 1; i < *argc; ++i)
    {
      if (strncasecmp ("--lash-server=", (*argv)[i], 14) == 0)
        {
          LASH_DEBUGARGS ("setting server from command line: '%s'", (*argv)[i] + 14);
      ...
      if (strncasecmp ("--lash-project=", (*argv)[i], 15) == 0)
        {
          LASH_DEBUGARGS ("setting project from command line: '%s'", (*argv)[i] + 15);
      ...
      if (strncmp ("--lash-id=", (*argv)[i], 10) == 0)
        {
          ...
          LASH_DEBUGARGS ("setting id from command line: '%s'", (*argv)[i] + 10);
      ...
      if (strncmp ("--lash-no-autoresume", (*argv)[i], 20) == 0)
        {
          LASH_PRINT_DEBUG ("setting LASH_No_Autoresume flag from command line");
          ...
          lash_protocol_t protocol)
{
  ...
  if (!str)
    {
      ...
      if (str)
        ...
        lash_connect_params_set_working_dir (connect_params, getenv ("HOME"));
    }
  else
    {
      lash_connect_params_set_working_dir (connect_params, str);
  ...
  LASH_DEBUGARGS ("protocol version for connect: %s", lash_protocol_string (protocol));
  ...
  if (err)
    {
      fprintf (stderr, "%s: could not connect to server '%s' - disabling lash\n",
  ...
  err = pthread_create (&client->recv_thread, NULL, lash_comm_recv_run, client);
  if (err)
    {
      fprintf (stderr, "%s: error creating recieve thread - disabling lash: %s\n",
  ...
  err = pthread_create (&client->send_thread, NULL, lash_comm_send_run, client);
  if (err)
    {
      fprintf (stderr, "%s: error creating send thread - disabling lash: %s\n",
               __FUNCTION__, strerror (err));
  ...
  if (client->events_in)
    {
      ...
      client->events_in =
        lash_list_remove (client->events_in, event);
    }
  ...
  if (client->configs_in)
    {
      ...
      client->configs_in =
        lash_list_remove (client->configs_in, config);
    }
...
lash_send_comm_event (lash_client_t * client, lash_comm_event_t * event)
{
  ...
  client->comm_events_out = lash_list_append (client->comm_events_out, event);
