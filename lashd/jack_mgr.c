...
static void jack_mgr_new_client_port (jack_mgr_t * jack_mgr, const char * port_name_port, jack_mgr_client_t * client);
...
static void
jack_mgr_port_registration_cb (jack_port_id_t port_id, int registered, void * data)
{
  jack_mgr_t * jack_mgr;
  ssize_t err;
  char port_notify = 1;
  ...
  err = write (jack_mgr->callback_write_socket, &port_notify, sizeof (port_notify));
  if (err == -1)
    {
      fprintf (stderr, "%s: could not send data to jack manager callback, aborting: %s\n",
...
jack_mgr_init_jack (jack_mgr_t * jack_mgr)
{
  ...
  err = jack_set_port_registration_callback (jack_mgr->jack_client, jack_mgr_port_registration_cb, jack_mgr);
  if (err)
    {
  ...
  err = jack_set_graph_order_callback (jack_mgr->jack_client, jack_mgr_graph_reorder_cb, jack_mgr);
  if (err)
    {
  ...
  jack_on_shutdown (jack_mgr->jack_client, jack_mgr_shutdown_cb, jack_mgr->server);
...
jack_mgr_new (server_t * server)
{
  ...
  pthread_create (&jack_mgr->callback_thread, NULL, jack_mgr_callback_run, jack_mgr);
      ...
      if (strcmp (client->name, get_jack_client_name_only (fport->name)) == 0)
        {
          ...
          LASH_DEBUGARGS ("resuming previously foreign port '%s'", fport->name);
          jack_mgr_new_client_port (jack_mgr, port_name, client);
          free (port_name);
          
          jack_mgr->foreign_ports = lash_list_remove (jack_mgr->foreign_ports, fport);
  ...
  for (list = jack_mgr->foreign_ports; list; list = lash_list_next (list))
    {
      ...
      if (id == port->id)
        {
          jack_mgr->foreign_ports = lash_list_remove (jack_mgr->foreign_ports, port);
...
jack_mgr_resume_patch (jack_mgr_t * jack_mgr, jack_patch_t * patch)
{
  ...
  if (err)
    {
      LASH_DEBUGARGS ("could not (yet?) resume patch %s", jack_patch_get_desc (patch));
    }
  else
 ...
 */
static void
jack_mgr_new_client_port (jack_mgr_t * jack_mgr, const char * port, jack_mgr_client_t * client)
{
  lash_list_t * node;
  ...
  for (node = client->old_patches; node; )
    {
      ...
      if (err)
        {
          LASH_DEBUGARGS ("could not resume jack patch '%s' as the other client hasn't registered yet",
      ...
      if ( (strcmp (client->name, unset_patch->src_client) == 0 &&
        ...
        {
	  LASH_DEBUGARGS ("attempting to resume patch %s", jack_patch_get_desc (unset_patch));
...
jack_mgr_port_notification (jack_mgr_t * jack_mgr, jack_port_id_t port_id)
{
  ...
  if (!jack_port)
    {
      fprintf (stderr, "%s: jack port registration notification for non-existant port\n", __FUNCTION__);
      return;
    }
  
  LASH_DEBUGARGS ("recieved port registration notification for port '%u'", port_id);
    
  client_name = lash_strdup (jack_port_name (jack_port));
  ptr = strchr (client_name, ':');
  if (!ptr)
    {
      fprintf (stderr, "%s: port name '%s' contains no client name/port name seperator\n",
...
lash_list_t *
jack_mgr_get_patches_with_type (const char * client_name, jack_client_t * jack_client, int type_flags)
{
  jack_patch_t * patch;
  ...
  port_names = (char **) jack_get_ports (jack_client, port_name_regex, NULL, type_flags);
  ...
  for (i = 0; port_names[i]; i++)
    {
      ...
      connected_port_names = (char **) jack_port_get_all_connections (jack_client, port);
      ...
      for (j = 0; connected_port_names[j]; j++)
        {
          ...
          LASH_DEBUGARGS ("found patch: '%s'/'%s'", jack_patch_get_src (patch),  jack_patch_get_dest (patch));
        }
...
jack_mgr_get_patches (jack_mgr_t * jack_mgr, jack_mgr_client_t * client)
{
  ...
  patches = jack_mgr_get_patches_with_type (
              client->name,
  ...
  for (list = jack_mgr->foreign_ports; list;)
    {
      ...
      port = jack_port_by_id (jack_mgr->jack_client, jack_fport_get_id (fport));
...
jack_mgr_read_callback (jack_mgr_t * jack_mgr)
{
  ...
  err = read (jack_mgr->callback_read_socket, &port_notify, sizeof (port_notify));
  if (err == -1)
    {
      fprintf (stderr, "%s: could not read data from callback socket; aborting: %s\n",
               __FUNCTION__, strerror (errno));
      abort ();
    }
  ...
  if (port_notify)
    {
      ...
      err = read (jack_mgr->callback_read_socket, &registered, sizeof (registered));
      if (err == -1)
        {
          fprintf (stderr, "%s: could not read data from callback socket; aborting: %s\n",
                   __FUNCTION__, strerror (errno));
  ...
  while (!jack_mgr->quit)
    {
      ...
      err = select (jack_mgr->callback_read_socket + 1, &select_set, NULL, NULL, &timeout);
