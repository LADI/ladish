...
void
server_lash_event_client_name (project_t * project, client_t * client, client_t * query_client, const char * name)
{
  if (name)
  ...
  else
    {
      ...
      conn_mgr_send_client_lash_event (project->server->conn_mgr, query_client->conn_id, event);
    }
}

void
server_lash_event_jack_client_name (project_t * project, client_t * client, client_t * query_client, const char * name)
{
  if (name)
    {
      ...
      if (client->jack_client_name)
        {
          fprintf (stderr, "%s client '%s' sent more than one jack client name\n",
      ...
      LASH_DEBUGARGS ("adding client '%s' to jack manager with client name '%s'", client_get_id_str (client), name);
      jack_mgr_lock (project->server->jack_mgr);
      jack_mgr_add_client (project->server->jack_mgr, client->id, name, client->jack_patches);
      jack_mgr_unlock (project->server->jack_mgr);
      client->jack_patches = NULL; 
      LASH_PRINT_DEBUG ("added client");

      server_notify_interfaces (project, client, LASH_Jack_Client_Name, name);
    }
  else
    {
      lash_event_t * event;
      ...
      conn_mgr_send_client_lash_event (project->server->conn_mgr,
                                      query_client->conn_id,
                                      event);
    }
}

void
server_lash_event_alsa_client_id   (project_t * project, client_t * client, client_t * query_client, const char * string)
{
  if (string)
    {
      ...
      if (client->alsa_client_id)
        {
          fprintf (stderr, "%s: client '%s' sent more than one alsa client id\n",
                   __FUNCTION__, client_get_id_str (client));
          return;
        }
      ...
      alsa_mgr_add_client (project->server->alsa_mgr, client->id, alsa_id, client->alsa_patches);
      alsa_mgr_unlock (project->server->alsa_mgr);
      client->alsa_patches = NULL;

      server_notify_interfaces (project, client, LASH_Alsa_Client_ID, string);
    }
  else
    {
      lash_event_t * event;
...
server_lash_event_project_add          (server_t * server, const char * dir)
{
  ...
  if (project_name_exists (server->projects, project->name))
    {
      ...
      fprintf (stderr, "%s: changing project name for project in dir '%s' to '%s' to avoid clashing with existing project name '%s'\n",
  ...
  if (err == -1)
    {
      fprintf (stderr, "%s: could not get local host name, using 'localhost' instead: %s\n",
  ...
  for (node = project->lost_clients; node; node = lash_list_next (node))  
    {
      ...
      lash_exec_params_set_args        (exec_params, client->argc, (const char * const *) client->argv);
...
void
server_lash_event_project_dir (project_t * project, client_t * client, const char * dir)
{
  if (dir)
  ...
  else
    {
      ...
      conn_mgr_send_client_lash_event (project->server->conn_mgr, client->conn_id, event);
    }
}
