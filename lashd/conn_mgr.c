  ...
  for (list = conn_mgr->open_connections; list; list = lash_list_next (list))
    {
      ...
      LASH_DEBUGARGS ("closing open connection %ld ('%s')", conn->id, conn_get_str_id (conn));
      conn_destroy (conn);
    }
  ...
  for (list = conn_mgr->connections; list; list = list->next)
    {
      ...
      LASH_DEBUGARGS ("closing connection %ld ('%s')", conn->id, conn_get_str_id (conn));
      conn_destroy (conn);
    }
...
conn_mgr_set_socket_opts (int sock)
{
  ...
  if (err == -1)
    {
      fprintf (stderr, "%s: could not set close-on-exec on listen socket: %s\n",
               __FUNCTION__, strerror (errno));
      return -1;
    }
...
conn_mgr_bind_socket (int * sock, struct addrinfo * addr)
{
  ...
  *sock = socket (addr->ai_family, addr->ai_socktype, addr->ai_protocol);
  if (*sock == -1)
    {
      LASH_DEBUGARGS ("could not create socket with params domain=%d, type=%d, protocol=%d: %s",
		     addr->ai_family, addr->ai_socktype, addr->ai_protocol, strerror (errno));
      return -1;
    }
...
conn_mgr_start            (conn_mgr_t * conn_mgr)
{
  ...
  err = pthread_create (&conn_mgr->recv_thread, NULL, conn_mgr_recv_run, conn_mgr);
  if (err)
    {
...
conn_mgr_send_client_event (conn_mgr_t * conn_mgr, server_event_t * event)
{
  ...
  conn_mgr->client_events = lash_list_append (conn_mgr->client_events, event);
...
void
conn_mgr_send_client_lash_event       (conn_mgr_t * conn_mgr, unsigned long conn_id, lash_event_t * lash_event)
{
  server_event_t * server_event;
  
  if (!lash_event) return;
...
void
conn_mgr_send_client_lash_config       (conn_mgr_t * conn_mgr, unsigned int conn_id, lash_config_t * lash_config)
{
  server_event_t * server_event;
  
  if (!lash_config) return;
...
void
conn_mgr_send_client_lash_comm_event (conn_mgr_t * conn_mgr, unsigned int conn_id, lash_comm_event_t * event)
{
  server_event_t * server_event;
...
void
conn_mgr_send_server_lash_event       (conn_mgr_t * conn_mgr, unsigned long conn_id, lash_event_t * lash_event)
{
  server_event_t * server_event;
...
void
conn_mgr_send_server_lash_config       (conn_mgr_t * conn_mgr, unsigned int conn_id, lash_config_t * lash_config)
{
  server_event_t * server_event;
...
void
conn_mgr_send_server_lash_comm_event (conn_mgr_t * conn_mgr, unsigned int conn_id, lash_comm_event_t * event)
{
  server_event_t * server_event;
...
conn_mgr_accept_connection (conn_mgr_t * conn_mgr)
{
  ...
  if (conn->socket == -1)
    {
      fprintf (stderr, "%s: error accepting socket connection from '%s': %s\n",
               __FUNCTION__, conn_get_str_id (conn), strerror (errno));
      conn_destroy (conn);
      return;
    }
  LASH_DEBUGARGS ("accepted connection from '%s' with id %ld", conn_get_str_id (conn), conn->id);
  ...
  if (err == -1)
    {
      fprintf (stderr, "%s: could not set close-on-exec on connection %ld ('%s'): %s\n",
               __FUNCTION__, conn->id, conn_get_str_id (conn), strerror (errno));
      exit (1);
    }
  ...
  conn_mgr->open_connections = lash_list_append (conn_mgr->open_connections, conn);
}
...
conn_mgr_connect_client (conn_mgr_t * conn_mgr, conn_t * conn)
{
  ...
  if (err == -1 ||
      ...
      !LASH_PROTOCOL_IS_VALID (event->event_data.connect->protocol_version))
    {
      ...
      if (err == -1)
        fprintf (stderr, "%s: there was a recieve error from connection '%s': disconnecting client\n",
                 __FUNCTION__, conn_get_str_id (conn));

      else if (err == -2)
        fprintf (stderr, "%s: connection '%s' closed before sending Connect event\n",
                 __FUNCTION__, conn_get_str_id (conn));

      else if (err == -3)
        fprintf (stderr, "%s: connection '%s' is using the wrong low-level protocol version\n",
                 __FUNCTION__, conn_get_str_id (conn));

      else if (lash_comm_event_get_type (event) != LASH_Comm_Event_Connect)
        fprintf (stderr, "%s: connection '%s' sent an event (of type %d) that wasn't Connect before it was connected; removing\n",
                 __FUNCTION__, conn_get_str_id (conn), lash_comm_event_get_type (event));

      else if (!LASH_PROTOCOL_IS_VALID (event->event_data.connect->protocol_version))
        {
          lash_comm_event_t mismatch_event;

          fprintf (stderr, "%s: connection '%s' is using protocol %s; disconnecting\n",
                   __FUNCTION__, conn_get_str_id (conn), lash_protocol_string (event->event_data.connect->protocol_version));
          
          lash_comm_event_set_protocol_mismatch (&mismatch_event, LASH_PROTOCOL_VERSION);
          lash_comm_send_event (conn->socket, &mismatch_event);
        }
      
      lash_comm_event_destroy (event);
      FD_CLR(conn->socket, &conn_mgr->sockets);
      conn_mgr->open_connections = lash_list_remove (conn_mgr->open_connections, conn);
      conn_destroy (conn);
      return;
    }
  ...
  LASH_DEBUGARGS   ("  protocol:       %s",   lash_protocol_string (event->event_data.connect->protocol_version));
  ...
  server_event_set_lash_connect_params (server_event, event->event_data.connect);
...
void
conn_mgr_disconnect_client (conn_mgr_t * conn_mgr, conn_t * conn, int notify_server)
{
...
void
conn_mgr_deal_with_event (conn_mgr_t * conn_mgr, conn_t * conn, lash_comm_event_t * event)
{
  switch (lash_comm_event_get_type (event))
    {
    case LASH_Comm_Event_Event:
      conn_mgr_send_server_lash_event (conn_mgr, conn->id, lash_comm_event_take_event (event));
      lash_comm_event_destroy (event);
      break;
    case LASH_Comm_Event_Config:
      conn_mgr_send_server_lash_config (conn_mgr, conn->id, lash_comm_event_take_config (event));
      lash_comm_event_destroy (event);
      break;
...
conn_mgr_recv_event (conn_mgr_t * conn_mgr, conn_t * conn)
{
  ...
  if (err == -2 ||
      lash_comm_event_get_type (event) == LASH_Comm_Event_Close)
    {
...
conn_mgr_read_socket (conn_mgr_t * conn_mgr, int socket)
{
  ...
  fprintf (stderr, "%s: could not find a connection with socket %d!  this should, like, never happen.  you shouldn't be reading this.  if you are, bad things have happened.  your computer will likely melt after about 20 minutes.",
           __FUNCTION__, socket);
  abort();
}
      ...
      if (conn_get_pinged (conn))
        {
          if (conn_ping_timed_out (conn, now))
            {
              fprintf (stderr, "%s: connection '%ld' has not responded to ping for %ld seconds, disconnecting it\n",
  ...
  while (!conn_mgr->quit)
    {
      ...
      err = select (conn_mgr->fd_max + 1, &sockets, NULL, NULL, &select_timeout);
      if (err == -1)
        {
          if (errno == EINTR)
            continue;
...
void
conn_mgr_send_lash_comm_event_to_client (conn_t * conn, lash_comm_event_t * lash_comm_event)
{
  int err;
...
conn_mgr_send_disconnect_client (conn_mgr_t * conn_mgr, unsigned long conn_id)
{
  ...
  pthread_mutex_unlock (&conn_mgr->connections_lock);
  fprintf (stderr, "%s: request from server to remove unknown connection %ld\n",
           __FUNCTION__, conn_id);
}

void
conn_mgr_send_server_event_to_client (conn_mgr_t * conn_mgr, server_event_t * server_event)
{
  ...
  switch (server_event->type)
    ...
    case Client_Event:
      ...
      lash_comm_event_set_event (lash_comm_event, server_event_take_lash_event (server_event));
      break;

    case Client_Config:
      lash_comm_event = lash_comm_event_new ();
      lash_comm_event_set_config (lash_comm_event, server_event_take_lash_config (server_event));
      break;
    ...
    case Client_Connect:
    default:
      fprintf (stderr, "%s: recieved unknown send request of type %d from server\n",
  ...
  while (!conn_mgr->quit)
    {
      ...
      if (list)
        ...
        {
          pthread_cond_wait (&conn_mgr->client_event_cond, &conn_mgr->client_event_lock);
          list = conn_mgr->client_events;
          conn_mgr->client_events = NULL;
        }
