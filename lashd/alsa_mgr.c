...
alsa_mgr_init_alsa(alsa_mgr_t * alsa_mgr)
{
	...
	err = snd_seq_connect_from(alsa_mgr->seq, snd_seq_port_info_get_port(port_info), SND_SEQ_CLIENT_SYSTEM,	/* the "System" kernel client */
					...
					unsigned char alsa_client_id, lash_list_t * alsa_patches)
{
...
#ifdef LASH_DEBUG
	{
		...
		LASH_DEBUGARGS("added client with alsa client id '%d' and patches:",
					   alsa_client_id);
...
alsa_mgr_remove_client(alsa_mgr_t * alsa_mgr, uuid_t id)
{
	...
	LASH_DEBUGARGS("removed client with alsa client id %d",
				   client->client_id);
...
alsa_mgr_resume_patch(alsa_mgr_t * alsa_mgr, alsa_patch_t * patch)
{
	...
	if (err) {
		...
		printf
			("Connected ALSA port %d on client %d to port %d on client %d\n",
						 ...
						 unsigned char port)
{
	...
	for (list = client->old_patches; list;) {
		...
		if (err) {
			LASH_DEBUGARGS
				("could not resume patch '%s' because the other client has not registered\n",
		...
		if ((client->client_id == alsa_patch_get_src_client(unset_patch) &&
			...
			LASH_DEBUGARGS
				("attempting to resume patch for client with alsa client id %d",
				 client->client_id);
	...
	LASH_DEBUGARGS("resumed all patches for client with alsa client id %d",
				   client->client_id);
					  ...
					  unsigned char port)
{
	...
	for (list = alsa_mgr->foreign_ports; list; list = lash_list_next(list)) {
		...
		if (fport->client == client && fport->port == port) {
			...
			LASH_DEBUGARGS("removing foreign port with port %d and client %d",
						   client, port);
...
alsa_mgr_redo_client_patches(alsa_mgr_t * alsa_mgr, alsa_client_t * client)
{
...
#ifdef LASH_DEBUG
	{
		...
		if (list) {
			LASH_DEBUGARGS
				("got alsa patches for client with alsa client id %d:",
				 alsa_client_get_client_id(client));
			...
			LASH_DEBUGARGS
				("no alsa patches for client with alsa client id %d",
				 alsa_client_get_client_id(client));
		}
	}
#endif
