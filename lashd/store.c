...
store_open (store_t * store)
{
  ...
  if (err == -1)
    {
      fprintf (stderr, "%s: error reading from info file for store '%s': %s\n",
               __FUNCTION__, store->dir, strerror (errno));
      if (line) free (line);
      return 1;
    }
...
store_remove_config (store_t * store, const char * key)
{
  ...
  for (list = store->unstored_configs; list; list = lash_list_next (list))
    {
      ...
      if (strcmp (lash_config_get_key (config), key) == 0)
        {
          store->unstored_configs = lash_list_remove (store->unstored_configs, config);
          lash_config_destroy (config);
          break;
        }
    }
  ...
  store->removed_configs = lash_list_append (store->removed_configs, lash_strdup (key));
}
...
store_set_config (store_t * store, const lash_config_t * config)
{
  ...
  if (!list) {
    store->keys = lash_list_append (store->keys, lash_strdup (lash_config_get_key (config)));
    store->key_count++;
  }
  ...
  for (list = store->unstored_configs; list; list = lash_list_next (list))
    {
      ...
      if (strcmp (lash_config_get_key (config), lash_config_get_key (exconfig)) == 0)
        {
          store->unstored_configs = lash_list_remove (store->unstored_configs, exconfig);
  ...
  store->unstored_configs = lash_list_append (store->unstored_configs, lash_config_dup (config));
}
...
store_write_config (store_t * store, const lash_config_t * config)
{
  ...
  config_file = creat (store_get_config_filename (store, lash_config_get_key (config)),
                       S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
  
  if (config_file == -1)
    {
      fprintf (stderr, "%s: error opening config file '%s' for writing in store '%s'!: %s\n", 
  ...
  if (written == -1 || written < sizeof (size))
    {
      fprintf (stderr, "%s: error writing to config file '%s' in store '%s'!: %s\n",
               __FUNCTION__, lash_config_get_key (config), store->dir, strerror (errno));
      close (config_file);
      return 1;
    }
  ...
  if (size > 0)
    {
      written = write (config_file, lash_config_get_value (config), lash_config_get_value_size (config));
      if (written == -1 || written < sizeof (size))
        {
          fprintf (stderr, "%s: error writing to config file '%s' in store '%s'!: %s\n",
                   __FUNCTION__, lash_config_get_key (config), store->dir, strerror (errno));
  ...
  if (err == -1)
    {
      fprintf (stderr, "%s: error close config file '%s' in store '%s'!: %s\n",
               __FUNCTION__, lash_config_get_key (config), store->dir, strerror (errno));
      return 1;
    }
  ...
  for (list = store->unstored_configs; list;)
    {
      ...
      if (err)
        ...
        store->unstored_configs = lash_list_remove (store->unstored_configs, old_list);
    }
  ...
  for (list = store->removed_configs; list;)
    {
      ...
      if (err)
        ...
        store->removed_configs = lash_list_remove (store->removed_configs, old_list);
    }
...
store_write_info_file (store_t * store)
{
  ...
  if (err < 0)
    {
      fprintf (stderr, "%s: error writing to info file for store '%s'!: %s\n",
               __FUNCTION__, store->dir, strerror (errno));
      fclose (info_file);
      return 1;
    }
  ...
  while (list)
    {
      ...
      LASH_DEBUGARGS ("removing config with key '%s', filename '%s'", key, filename);
...
store_get_config (store_t * store, const char * key)
{
  ...
  if (config_file == -1)
    {
      fprintf (stderr, "%s: could not open config file '%s' for reading!: %s\n",
               __FUNCTION__, store_get_config_filename (store, key), strerror (errno));
      return NULL;
    }
  ...
  if (err == -1 || err < sizeof (size))
    {
      fprintf (stderr, "%s: error reading value size in config file '%s': %s\n",
