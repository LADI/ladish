...
server_send_event(server_t * server, server_event_t * server_event)
{
	...
	server->server_events =
		lash_list_append(server->server_events, server_event);
...
int
find_client_in_list_by_conn_id(lash_list_t * client_list,
							   unsigned long conn_id, client_t ** client_ptr)
{
	client_t *client;
...
void
server_find_project_and_client_by_conn_id(server_t * server,
										  ...
										  client_t ** client_ptr)
{
	...
	while (list) {
		...
		found =
			find_client_in_list_by_conn_id(project->clients, conn_id,
										   client_ptr);
	...
	found =
		find_client_in_list_by_conn_id(server->interfaces, conn_id,
									   client_ptr);
	if (found) {
		if (project_ptr)
			*project_ptr = NULL;
...
void
server_notify_interfaces(project_t * project, client_t * client,
						 enum LASH_Event_Type type, const char *str)
{
	lash_list_t *node;
	lash_event_t *event;
	client_t *interface;

	for (node = project->server->interfaces; node;
...
server_create_new_project_name(server_t * server)
{
	...
	do {
		...
		LASH_DEBUGARGS("trying project name '%s', dir '%s'", new_name,
					   lash_get_fqn(lash_get_fqn
									(getenv("HOME"), server->default_dir),
									new_name));
	}
	while (project_name_exists(server->projects, new_name) ||
		   access(lash_get_fqn
				  (lash_get_fqn(getenv("HOME"), server->default_dir),
				   new_name), F_OK) == 0);
...
server_create_new_project(server_t * server, const char *name)
{
	...
	if (!name || !strlen(name)) {	/* create a project name */
...
server_close_project(server_t * server, project_t * project)
{
	...
	server_notify_interfaces(project, NULL, LASH_Project_Remove,
							 project->name);
...
server_add_interface(server_t * server, client_t * interface)
{
	...
	for (pnode = server->projects; pnode; pnode = lash_list_next(pnode)) {
		...
		conn_mgr_send_client_lash_event(server->conn_mgr, interface->conn_id,
										event);
		...
		for (clnode = project->clients; clnode;
			...
			lash_event_set_project(event, project->name);
			lash_event_set_client_id(event, client->id);
			conn_mgr_send_client_lash_event(server->conn_mgr,
											interface->conn_id, event);

			if (client->name) {
				LASH_DEBUGARGS("sending client's name to interface: %s",
							   client->name);
			...
			if (client->jack_client_name) {
				LASH_DEBUGARGS
					("sending client's jack client name to interface: %s",
			...
			if (client->alsa_client_id) {
				...
				LASH_DEBUGARGS
					("sending client's alsa client id to interface: %u",
					 client->alsa_client_id);
...
server_add_client(server_t * server, client_t * client)
{
	...
	if (!project) {				/* try and find an existing project */
		if ((!client->requested_project || !strlen(client->requested_project))
			...
			project =
				server_create_new_project(server, client->requested_project);
		}
	}
...
server_event_client_disconnect(server_t * server, server_event_t * event)
{
	...
	server_find_project_and_client_by_conn_id(server, event->conn_id,
											  &project, &client);
	if (!client) {
		fprintf(stderr,
				"%s: recieved disconnect for connection id %ld, but no client exists for it :/\n",
...
server_event_client_event(server_t * server, server_event_t * server_event)
{
	...
	server_find_project_and_client_by_conn_id(server, server_event->conn_id,
											  &project, &client);
	if (!client) {
		fprintf(stderr,
				"%s: recieved server event from connection manager for unknown client with connection id %ld\n",
	...
	if (!project) {
		...
		project =
			server_find_project_by_name(server,
										lash_event_get_project(event));

		if (project && !uuid_is_null(event->client_id)) {
			ref_client = project_get_client_by_id(project, event->client_id);
			if (!ref_client) {
				fprintf(stderr,
						"%s: recieved event from server interface with non-existant client "
	...
	case LASH_Client_Name:
		if (CLIENT_SERVER_INTERFACE(client))
			server_lash_event_client_name(project, ref_client, client,
										  lash_event_get_string(event));
		else
			server_lash_event_client_name(project, client, client,
										  lash_event_get_string(event));
		break;
	case LASH_Jack_Client_Name:
		if (CLIENT_SERVER_INTERFACE(client))
			server_lash_event_jack_client_name(project, ref_client, client,
											   lash_event_get_string(event));
		else
			server_lash_event_jack_client_name(project, client, client,
											   lash_event_get_string(event));
		break;
	case LASH_Alsa_Client_ID:
		if (CLIENT_SERVER_INTERFACE(client))
			server_lash_event_alsa_client_id(project, ref_client, client,
											 lash_event_get_string(event));
		else
			server_lash_event_alsa_client_id(project, client, client,
											 lash_event_get_string(event));
		break;
	...
	case LASH_Project_Dir:
		server_lash_event_project_dir(project, client,
									  lash_event_get_string(event));
		break;
...
server_event_client_config(server_t * server, server_event_t * event)
{
	...
	/* find the project and client */
	server_find_project_and_client_by_conn_id(server, event->conn_id, NULL,
											  &client);
	if (!client) {
		fprintf(stderr,
				"%s: recieved server event from connection manager for unknown client with connection id %ld\n",
	...
	if (!CLIENT_CONFIG_DATA_SET(client)) {
		fprintf(stderr,
				"%s: client '%s' sent us a config but doesn't describe itself as configured with a data set\n",
