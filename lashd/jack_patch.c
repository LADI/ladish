...
{
  jack_patch_set_client_and_port (patch, src, jack_patch_set_src_client, jack_patch_set_src_port);
}

void
jack_patch_set_dest       (jack_patch_t * patch, const char * dest)
{
  jack_patch_set_client_and_port (patch, dest, jack_patch_set_dest_client, jack_patch_set_dest_port);
}
...
jack_patch_get_src  (jack_patch_t * patch)
{
  if (!patch->src_client)
    ...
    return jack_patch_recreate_port_name (patch->src_client, patch->src_port);
}
...
jack_patch_get_dest (jack_patch_t * patch)
{
  if (!patch->dest_client)
    ...
    return jack_patch_recreate_port_name (patch->dest_client, patch->dest_port);
}
...
static void
jack_patch_set_client_id (lash_list_t * jack_mgr_clients, const char * patch_client_name, uuid_t id)
{
  jack_mgr_client_t * client;
  
  /* find the client */
  for (; jack_mgr_clients; jack_mgr_clients = lash_list_next (jack_mgr_clients))
    {
...
{
  jack_patch_set_client_id (jack_mgr_clients, patch->src_client, patch->src_client_id);
  jack_patch_set_client_id (jack_mgr_clients, patch->dest_client, patch->dest_client_id);
}
...
jack_patch_create_xml (jack_patch_t * patch, xmlNodePtr parent)
{
  ...
  if (uuid_is_null (patch->src_client_id))
    xmlNewChild (patchxml, NULL, BAD_CAST "src_client", BAD_CAST patch->src_client);
  else
    {
      char id[37];
      uuid_unparse (patch->src_client_id, id);
  ...
  xmlNewChild (patchxml, NULL, BAD_CAST "src_port", BAD_CAST patch->src_port);


  if (uuid_is_null (patch->dest_client_id))
    xmlNewChild (patchxml, NULL, BAD_CAST "dest_client", BAD_CAST patch->dest_client);
  else
    {
      char id[37];
      uuid_unparse (patch->dest_client_id, id);
  ...
  xmlNewChild (patchxml, NULL, BAD_CAST "dest_port", BAD_CAST patch->dest_port);
}
