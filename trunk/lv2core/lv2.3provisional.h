 ...
 *
 * Plugins may process any data type.  This specification defines two basic
 * types: 'control rate' or 'audio rate' floating point data.  Plugins have
 ...
 *
 * Plugins reside in shared object files suitable for dynamic linking
 * by dlopen() and family. The object file provides a number of 'plugin
...
typedef struct _LV2_Descriptor { 
	 ...
	 * signature', meaning a port with the given symbol (but not necessarily
	 * index) always refers to the 'same' port on any version of the plugin
	 * with that URI.  Plugin authors MUST change the plugin URI if
	 * port symbols are 'broken' or removed.  Ports are the "same" in this
	 * context if they support the same data types and have the same
	 * semantics (e.g. "set filter cutoff").
	 *
	 * There is no explicit versioning in LV2 as this rule ensures
	 * compatibility between different versions of the same plugin.
	 * If the 'port signature' changes the plugin URI MUST change, and the
	 * plugins is NOT the 'same' plugin.  It is a good idea to include
	 * version numbers of some sort in plugin URIs for this reason.
	 *
	 * When serializing session/patch/etc files, hosts MUST:
	 *  - Refer to a plugin by its URI only (not e.g. file name or index).
	 *  - Refer to a port by its symbol only (not index)
	 *
	 * Rationale:  This ensures files will not break when a plugin has been
