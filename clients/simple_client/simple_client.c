      ...
      if (event)
        {
          printf ("%s: got event of type %d with string '%s'\n",
                  __FUNCTION__, lash_event_get_type (event), lash_event_get_string (event));
          switch (lash_event_get_type (event))
          {
          case LASH_Save_Data_Set:
            config = lash_config_new_with_key ("test config");
            lash_config_set_value_string (config, "this is some configuration data");
          ...
          {
            FILE * config_file;
            config_file = fopen (lash_get_fqn (lash_event_get_string (event), "simple-client.data"), "w");
          ...
          case LASH_Restore_File:
          {
            ...
            config_file = fopen (lash_get_fqn (lash_event_get_string (event), "simple-client.data"), "r");
            if (config_file)
              {
      ...
      if (config)
        {
          printf ("%s: got config with key '%s', value_size %d\n",
                  __FUNCTION__, lash_config_get_key (config), lash_config_get_value_size (config));
