...
double phi[POLY], phi_mod[POLY], pitch, modulation, velocity[POLY],
       attack, decay, sustain, release, env_time[POLY], env_level[POLY], gain, sample_rate = 48000.0;
...
snd_seq_t * open_seq () {
    ...
    if (snd_seq_create_simple_port(seq_handle, "miniFMsynth",
        ...
        SND_SEQ_PORT_TYPE_APPLICATION|SND_SEQ_PORT_TYPE_SYNTH) < 0) {
        fprintf(stderr, "Error creating sequencer port.\n");
        exit(1);
    }

    printf ("Connected to ALSA sequencer with client id %d\n", snd_seq_client_id (seq_handle));
    lash_alsa_client_id (lash_client, (unsigned char) snd_seq_client_id (seq_handle));
...
open_jack ()
{
  ...
  jack_port = jack_port_register (jack_client, "out_1", JACK_DEFAULT_AUDIO_TYPE,
...
go_real_time ()
{
  ...
  if (err)
    fprintf (stderr, "%s: could not get scheduling parameters of jack thread\n", __FUNCTION__);
  else
    {
