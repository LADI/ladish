...
print_help()
{
	...
	printf
		(" -p, --pid                   Use the pid in the client string (default)\n");
	printf
		(" -e, --name <string>         Use the specified string in the client string\n");
	printf
		(" -b, --basic-name            Use just the program name in the client string\n");
	printf("\n");
	printf
		(" -m, --modulation <float>    Set the synth modulation parameter  [%g]\n",
		 MODULATION_DEFAULT);
	printf
		(" -n, --harmonic <int>        Set the synth harmonic parameter    [%d]\n",
		 HARMONIC_DEFAULT);
	printf
		(" -u, --subharmonic <int>     Set the synth subharmonic parameter [%d]\n",
		 SUBHARMONIC_DEFAULT);
	printf
		(" -t, --transpose <int>       Set the synth transpose parameter   [%d]\n",
		 TRANSPOSE_DEFAULT);
	printf
		(" -a, --attack <float>        Set the synth attack parameter      [%g]\n",
		 ATTACK_DEFAULT);
	printf
		(" -d, --decay <float>         Set the synth decay parameter       [%g]\n",
		 DECAY_DEFAULT);
	printf
		(" -s, --sustain <float>       Set the synth sustain parameter     [%g]\n",
		 SUSTAIN_DEFAULT);
	printf
		(" -r, --release <float>       Set the synth release parameter     [%g]\n",
		 RELEASE_DEFAULT);
	printf
		(" -g, --gain <float>          Set the synth gain                  [%g]\n",
		 GAIN_DEFAULT);
	printf("\n");
}
...
main(int argc, char **argv)
{
	...
	lash_client =
		lash_init(lash_args, "LASH Synth", flags, LASH_PROTOCOL(2, 0));
