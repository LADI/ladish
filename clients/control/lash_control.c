...
command_help ()
{
  ...
  printf ("  dir <dir>         Set the selected project's directory to <dir>\n");
...
/*  printf ("  tar  <tarball>    Pack the selected project into <tarball>\n");
  ...
  printf ("\n");
  printf ("  It is only necessary to type enough characters of a command to\n");
  ...
  for (pnode = control->projects; pnode; pnode = lash_list_next (pnode))
    {
      ...
      printf ("  %s in directory %s, %s\n",
	      project->name,
	      project->dir,
	      project->clients ? "clients:" : "no clients");
      ...
      for (clnode = project->clients; clnode; clnode = lash_list_next (clnode))
	{
	  ...
	  if (!client->alsa_client_id)
	    ...
	    sprintf (client_alsa_str, ", ALSA client ID: %u", (unsigned int) client->alsa_client_id);

	  if (!client->jack_client_name)
	    client_jack_str[0] = '\0';
	  else
	    sprintf (client_jack_str, ", JACK client name: %s", client->jack_client_name);

	  printf ("    %s%s%s\n", client_get_identity (client), client_jack_str, client_alsa_str);
	}
    }
}
...
command_remove (lash_control_t * control, char * arg)
{
  ...
  for (node = control->cur_project->clients; node; node = lash_list_next (node))
    {
...
deal_with_events (lash_control_t * control)
{
  ...
  while ( (event = lash_get_event (control->client)) )
	...
	project = NULL;
      ...
      if (!uuid_is_null (client_id) && lash_event_get_type (event) != LASH_Client_Add)
 	{
	  for (node = control->projects; node; node = lash_list_next (node))
	    for (clnode = ((project_t *)node->data)->clients; clnode; clnode = lash_list_next (clnode))
	      {
		client = (client_t *) clnode->data;
	...
	case LASH_Alsa_Client_ID:
	  printf ("* Client '%s' set its ALSA client ID to '%u'\n",
		  client_get_identity (client), lash_str_get_alsa_client_id (event_str));
