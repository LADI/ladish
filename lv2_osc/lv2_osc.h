...
/** @file
 * This is an LV2 message specification, closely related (and in places
 * binary compatible) to OSC, which defines a buffer format for a sequence
 * of timestamped messages, and adds some indexing for performance, so that
 * accessors for messages and arguments are very fast:
 * O(1) and realtime safe, unless otherwise noted.
 */


/** Pad a size to a multiple of 32 bits */
inline static uint32_t lv2_osc_pad_size(uint32_t size) { return size + 3 - ((size-1) % 4); }
...
inline static const void* lv2_message_get_osc_message(LV2Message* msg)
	{ return (const void*)(&msg->data + lv2_osc_pad_size(sizeof(uint32_t) * msg->argument_count)); }

inline static const char* lv2_message_get_path(LV2Message* msg)
	{ return (const char*)(&msg->data + lv2_osc_pad_size(sizeof(uint32_t) * msg->argument_count)); }
...
/** Message Buffer.
 *
 * This is the contents of a plugin's message port buffer (ie the void* points
 * to a struct of this type).
 */
typedef struct {
...
/** Append a message from a raw OSC message */
